...
DEPLOY_USER="${DEPLOY_USER:-lavilla}"
...
install -d -o "$DEPLOY_USER" -g "$DEPLOY_USER" /opt/lavillasb
...
Point these DNS A records at this host before the first start, or Caddy cannot
issue certificates:
  api.<domain>  media.<domain>  storefront.<domain>
EOF
