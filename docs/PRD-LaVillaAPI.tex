...
\newcommand{\docdate}{08 de Septiembre de 2026}
\newcommand{\contractvalue}{\$1.500 a \$2.000\ USD (estimado)}
\newcommand{\contractperiod}{6 meses}
...
La plataforma se construyó como microservicios y no como una aplicación única. La razón no es preferencia sino aislamiento de carga: la sincronización del catálogo y el procesamiento de imágenes son lentos y ocurren a ráfagas, y en un solo proceso competirían por los mismos recursos que el checkout. Separados, el catálogo puede tardar sin degradar la compra.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.74\textheight,keepaspectratio]{diagrams/monolito-vs-microservicios.png}
\caption{Monolito frente a microservicios. En la opción A un proceso lento degrada al crítico; en la opción B cada servicio escala y falla por separado.}
\end{figure}
...
\subsection{Fase 4: Mantenimiento (Mes 1--6)}
\begin{itemize}
...
Mantenimiento & 6 meses & Monitoreo, bugs, seguridad, soporte \\
\hline
\end{tabularx}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.74\textheight,keepaspectratio]{diagrams/arquitectura-sistema.png}
\caption{Diagrama de arquitectura del sistema. Las cuatro capas y el flujo de una petición, desde el navegador hasta la persistencia.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.74\textheight,keepaspectratio]{diagrams/bloques-funcionales.png}
\caption{Diagrama de bloques funcionales. Las capacidades del sistema agrupadas por audiencia, con el servicio que implementa cada una.}
\end{figure}

\subsection{Diagrama de Comunicación}

Muestra cómo se comunican los componentes durante un checkout: la parte síncrona que el cliente espera, y la asíncrona que ocurre después por el bus de eventos.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.74\textheight,keepaspectratio]{diagrams/comunicacion.png}
\caption{Diagrama de comunicación. Los pasos 1 a 9 son REST síncrono; a partir del paso 10 el flujo continúa por RabbitMQ sin que el cliente espere.}
\end{figure}
...
Durante los \contractperiod\ de contrato, el mantenimiento sigue ciclos de 2--4 semanas:
