...
\newcommand{\contractvalue}{\$5.000.000\ COP}
...
{\Huge\bfseries\textcolor{sectioncolor}{Producto Requerido}\\[0.3cm]}
{\Huge\bfseries\textcolor{sectioncolor}{Documento (PRD)}\\[1.5cm]}
...
Duración & \contractperiod (12 meses) \\
...
Frontend (Next.js :3000)
  ...
  Gateway (Laravel :8010) ---[Sanctum Auth]---> PostgreSQL
        |
        +---> Catalog (:9002)    ---> PostgreSQL + MinIO + OpenAI
...
POST & \texttt{/search} & Búsqueda con lenguaje natural (LLM) \\
...
\subsubsection{Búsqueda con Lenguaje Natural}
\begin{itemize}
    \item Modelo: OpenAI GPT-4o-mini (configurable vía variable de entorno)
    \item Idiomas: Español e inglés
    \item Fallback: búsqueda por keywords con stemming español básico
    \item Extracción: categoría, subcategoría, precio, marca, palabras clave
    \item Lógica OR con scoring de relevancia
\end{itemize}
...
\texttt{OPENAI\_API\_KEY} & Clave API OpenAI (búsqueda NL) \\
...
\begin{enumerate}
    ...
    \item Búsqueda con lenguaje natural (español/inglés) potenciada por IA
...
\begin{enumerate}
    ...
    \item OpenAI API para búsqueda con lenguaje natural
    \item Google Drive para importación de catálogo (referencia)
\end{enumerate}

\subsection{Búsqueda con Inteligencia Artificial}
\begin{itemize}
    \item Parser LLM que extrae filtros estructurados de texto libre
    \item Categorías: decks, apparel, accessories, gear
    \item Subcategorías: 14 tipos (tenis, buzos, camisetas, trucks, ruedas, etc.)
    \item Extracción de: marca, rango de precio, color, talla
    \item Fallback con stemming español cuando no hay API key
    \item Scoring de relevancia OR con pesos por campo
\end{itemize}
...
\begin{itemize}
    ...
    \item Mejoras menores (hasta 10 horas/mes)
\end{itemize}
...
Incluye mantenimiento & Sí (hasta 10 horas/mes) \\
\hline
\end{tabularx}
...
\begin{enumerate}
    ...
    \item Búsqueda NL retorna resultados relevantes en español
