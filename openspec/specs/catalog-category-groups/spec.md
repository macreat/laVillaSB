...
> All 4 requirements are archived as live baseline behavior, verified against the compose stack (catalog :9002, gateway :8010) - 1500/1500 products carry `categoryGroup`, 0 NULL categories, backfill idempotent.
...
**Status**: ARCHIVED (as-built, verified live - 48/48 categories assigned, 0 NULLs; product counts 104 decks / 1294 apparel / 23 accessories / 79 gear; backfill re-run is a no-op).
...
**Status**: ARCHIVED (as-built, verified live - module `catalog/src/category_groups.py` is the single shared implementation; 13 parametrized unit cases + live DB edge rows: `Hardware y Accesorios` -> gear, `Long Board` -> decks, `Tenis / Talla 8Us -39Col / Todo` -> apparel, `Maletines - Canguros` -> accessories).
...
**Status**: ARCHIVED (as-built, verified live - 1500/1500 products with non-empty `categoryGroup`, 0 unknown values; gateway contract test asserts additive field; `test_product_out.py` covers stored/NULL/no-category/preserve-field cases).
...
**Status**: ARCHIVED (as-built, verified live - `build_category` tags on create, existing categories fetched by slug and never updated; live re-import preserved stored groups: 0 categories updated; `test_importer_tags.py` + `test_backfill_category_groups.py`).
